...
namespace IniParser.Parser
{
    public sealed class StringBuffer
    {
        public struct Range
        {
            ...
            readonly static Range _empty = new Range { start = 0, size = 0 };
        ...
        {
            _bufferIndexes.start = 0;
            _bufferIndexes.size = _buffer.Count;
        ...
        public string Substring(int startIndex, int size) 
        {
            return new string(_buffer.ToArray(),
                      _bufferIndexes.start + startIndex,
                      size);
        }

        public string Substring(Range range)
        {
            return Substring(range.start, range.size);
        }
